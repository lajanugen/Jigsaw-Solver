function [dests,allPaths] = findPaths2(mni,numOfParts,len,assembled)
...
if len == 1,
  ...
  orig = Dst;
	...
	% Discard path if it has more than one part that is already assembled
	assm = ismember(stages,assembled);
	inds = (sum(assm,2) == 1);% & assm(:,1);
