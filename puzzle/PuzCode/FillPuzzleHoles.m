...
function [newBlock,newRotBlock] = FillPuzzleHoles(Block, Rot, SCO, nr,nc,rotFlag, locked, givenPieces,v1v2Flag, addLots)
     ...
     %   neighborsThere = filled(neighbors_i);
      ...
      %  nei_code = nei_code(good);
        ...
        for ii = 1:1:numel(neighbors(:,1))
            [scoreMat] = getScoresForCands(SCO, choices, nei_ids(ii), nei_rot(ii), nei_code(ii), rotFlag)
            if(ii==1)
                totalScores = scoreMat;
            else
                totalScores = totalScores + scoreMat;
            end
            % okay, all neighbors have been considered, fill the hole!
        end
        [aa,bb] = min(totalScores(:));
        [iii,jjj] = ind2sub(size(totalScores), bb);
        choice= choices(iii);
        choicerot = jjj;
        %spot
       % sq = size(newBlock)
       % currval = newBlock(spot(1),spot(2) )
        newBlock(spot(1),spot(2) )= choice;
        newRotBlock(spot(1),spot(2)) = choicerot;
        
        
        
    else
        gogo = 0;  %quit the while loop.
    end
