...
%function [bmni,sort_cost,rankLim,cdif,md] = evalMet();
%function [pr,ps] = evalMet();
%function [str4,str4a,mni,agg_mni] = evalMet();
function [results] = evalMet(imNo);
...
image_ = double(image_);
imageSize = size(image_);
...
%system(['mkdir -p ' num2str(imNo)]);
%mnis = [];
%sco = mscore_mgc_orig(pcs3); 
%s = refine13(sco);
%
%[~,M] = sort(s,2,'descend');
%
%rnk = zeros(1,numOfParts);
%rnks = zeros(numOfParts,4);
%for i = 1:numOfParts 
%  for j = 1:4
%    n = correctNeighbor(i,j);
%    if n > 0 && n <= numOfParts,
%      fn = find(M(i,:,j) == n);
%      rnks(i,j) = fn;
%      rnk(fn) = rnk(fn) + 1;
...
%%save('mni','mni');
%pe = [];
%%numCorPairsmni(false,mni)
%%[~,incp,incr] = numCorPairsmni(false,mni);
mnis = [];
sco = mscore2_mgc_orig(pcs3);    
%[~,mni] = min(sco,[],2); 
%mni = squeeze(mni); 
%[~,incp,incr] = numCorPairsmni(false,mni);
%imp = displayIncorrect(incp,incr);
%imwrite(imp,[num2str(imNo) '/1.png'],'png');
...
%[~,incp,incr] = numCorPairsmni(false,mni);
%imp = displayIncorrect(incp,incr);
%imwrite(imp,[num2str(imNo) '/2.png'],'png');

mni = neighb_ref(mni); 
%[~,incp,incr] = numCorPairsmni(false,mni);
%imp = displayIncorrect(incp,incr);
%imwrite(imp,[num2str(imNo) '/3.png'],'png');

mnis = cat(3,mnis,squeeze(mni));

%sco = mscore_mgc_orig(pcs3);    s = refine13(sco); [~,mni] = max(s,[],2); mni = squeeze(mni); mni = neighb_ref(mni); mnis = cat(3,mnis,squeeze(mni));
...
%
%%rnks(rnks >= 20) = 0;
%textInserter = vision.TextInserter('%d','LocationSource','Input port','Color', [255, 255, 255],'FontSize', 10);
%row_locs = repmat(partSize*(1:rows)' - 3*partSize/4,1,cols);
%col_locs = repmat(partSize*(1:cols) - 3*partSize/4,rows,1);
%row_locs = row_locs';
%col_locs = col_locs';
%locs = [col_locs(:) row_locs(:)];
%J = step(textInserter, orig_image, int32(rnks(:,1)),int32(locs));
%figure,imshow(J);
%J = step(textInserter, orig_image, int32(rnks(:,3)),int32(locs));
%figure,imshow(J);
%pause
%
%
...
%
%%for it = 1:3
%%mnis = cat(3,squeeze(M(:,1,:)),squeeze(M(:,2,:)),squeeze(M(:,3,:)));
%for i = 1:10
%  mnis = cat(3,mnis,squeeze(M(:,i,:)));
%end

%fprintf('Score computation done \n');
%
%%mnis = cat(3,mni(:,1:4),Mni(:,1:4));
%save('mnis','mnis');
%mnis = [];
%load('mnis');
strs = [];
...
%[le1,m1] = linkStrengths_extended_ranks(mnis,numOfParts,6);
%[le2,m2] = linkStrengths_extended_ranks(mnis,numOfParts,4);
agg_mni(:,5) = (1:numOfParts)';

either_correct = zeros(numOfParts,4);
for i = 1:numOfParts
  for j = 1:4

    %for met = 1:5
    %  if isCorrect(i,mnis(i,j,met),j),
    %    either_correct(i,j) = mnis(i,j,met);
    %    %continue;
    %  end
    %end
    %[~,mx_ind] = max(le1{i,j} + le2{i,j});
    %[mx,mx_ind] = max(le1{i,j} + le2{i,j});
    %if mx == 0,
      %agg_mni(i,j) = mnis(i,j,5);
      ...
      %[mx,mx_ind] = max(le1{i,j});
      %%[mm,mm_ind] = max(strs(i,j,:));
      %if mx ~= 0,
      %  agg_mni(i,j) = m1{i,j}(mx_ind);
      %else
      %  agg_mni(i,j) = mnis(i,j,1);
      %end
...
%[perc0,incp,incr] = numCorPairsmni(false,agg_mni);
%displayIncorrect(incp,incr);
%
%[perc1,incp,incr] = numCorPairsmni(false,agg_mni);
%[~,incp,incr] = numCorPairsmni(false,agg_mni);
%displayIncorrect(incp,incr);
...
perc = numCorPairsmni(false,mni);
%imp = displayIncorrect(incp,incr);
%imwrite(imp,[num2str(imNo) '/4.png'],'png');

%perc = numCorPairsmni(false,mni);
%perc_ = numCorPairsmni(false,either_correct);
results = [results;perc]
%if it == 1
%  mnis = cat(3,mni(:,1:5),mnis);
%else
%  mnis(:,:,1) = mni(:,1:5);
%end

%end
%perc1 = numCorPairsmni(false,agg_mni);
%mni = neighb_ref(agg_mni); 
%perc2 = numCorPairsmni(false,mni);
%
%Perc = [Perc;perc1 perc2];
%[perc1 perc2]
%[pr,incp,incr] = numCorPairsmni(false,mni);
%pr
%displayIncorrect(incp,incr);
...
function ranks = scoreRanks(score,flag)
  ranks = zeros(numOfParts,4);
  if flag,  [sco_sort,sco_ind] = sort(score,2,'descend');
  else      [sco_sort,sco_ind] = sort(score,2,'ascend');
  end
  for i = 1:numOfParts
    for j = 1:4
      partsVec = sco_ind(i,:,j);
      rnk = find(partsVec == correctNeighbor(i,j));
      if isempty(rnk), ranks(i,j) = numOfParts+1;
      else ranks(i,j) = rnk;
      end
    end
  end
...
function v = correctNeighbor(u,r)
  if (r==1) v=u-1 ; end
  if (r==2) v=u+1 ; end
  if (r==3) v=u-cols; end
  if (r==4) v=u+cols; end
...
function imp = displayIncorrect_bkp(incorrect_parts,incorrect_relation) 
...
function imp = displayIncorrect(incorrect_parts,incorrect_relation) 
  impcs = zeros((partSize+2)*rows,(partSize+4)*cols,3);
  for index = 1 : numOfParts
  ...
  for index = 1:length(incorrect_parts)
    ...
    switch rel
      case 1, impcs(rowStartIndx:rowEndIndx,colStartIndx-2:colStartIndx-1,1) = bitmax;
      case 2, impcs(rowStartIndx:rowEndIndx,colEndIndx+1:colEndIndx+2,1) = bitmax;
      case 3, impcs(rowStartIndx-2:rowStartIndx-1,colStartIndx:colEndIndx,1) = bitmax;
      case 4, 
        impcs(rowEndIndx+1:rowEndIndx+2,colStartIndx:colEndIndx,1) = bitmax;
    end  
  end
...
function [convertedImage] = convertToUserSelectedColorScheme()
  if (colorScheme == 0)
    convertedImage = image_;
  elseif (colorScheme == 1)
    cformRgbToLab = makecform('srgb2lab');
    convertedImage = double(applycform(image_, cformRgbToLab));
  elseif (colorScheme == 2)
    convertedImage = rgb2hsv(image_);
  elseif (colorScheme == 3)
    convertedImage = rgb2gray(image_);
  else
...
function cutImageToParts()
  for index = 1 : numOfParts
    ...
    rgbPartsArray(:,:,:, index) = image_(rowStartIndex :rowEndIndex, colStartIndex :colEndIndex, :);
